...
public partial class Default2 : System.Web.UI.Page
{
    ...
    protected void LoginButton_Click(object sender, EventArgs e)
    {
        ...
        if (Page.IsValid)
        {
            ...
            //  QueryString = "SELECT (ISNULL(mr.First_Name,'')+' '+ISNULL(mr.Last_Name,'')) as Name, Membership_No AS MembershipNo, ai.RoleId as Role FROM MemberRegister mr INNER JOIN PHM_AccountInfo ai ON mr.Email_ID = ai.EmailId WHERE (MembershipNo = @UserName OR EmailId = @UserName OR MobileNo = @UserName) AND Password = @Password AND Status <> 'Inactive'";
            QueryString = "SELECT (ISNULL(mr.First_Name,'')+' '+ISNULL(mr.Last_Name,'')) as Name, Membership_No AS MembershipNo, ai.RoleId as Role,mr.Member_Type as MemberType FROM MemberRegister mr INNER JOIN PHM_AccountInfo ai ON mr.Email_ID = ai.EmailId WHERE (MembershipNo = @UserName OR EmailId = @UserName OR MobileNo = @UserName) AND Password = @Password AND Status <> 'Inactive'";
            ...
            try
            {
                ...
                {
                    FailureText.Text = "Incorrect Membership No./Email/Mobile No. and/or Password!";
                }
            }
